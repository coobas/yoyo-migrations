...
def test_v3_preserves_history_when_upgrading(backend):
    clear_database(backend)
    internalmigrations.upgrade(backend, version=1)
    v2insert = ("INSERT INTO {0.migration_table_quoted} (id, ctime) "
                "VALUES (:id, :when)".format(backend))

    with backend.transaction():
        backend.execute(v2insert,
                        {'id': 'migration-a',
                         'when': datetime(2000, 1, 1, 12)})
        backend.execute(v2insert,
                        {'id': 'migration-b',
                         'when': datetime(2000, 2, 1, 12)})

    internalmigrations.upgrade(backend, version=2)

    cursor = backend.execute(
        'SELECT migration_hash, migration_id, applied_at_utc '
        'FROM {0.migration_table_quoted} order by applied_at_utc'
        .format(backend))
    ...
    cursor = backend.execute(
        ...
        'FROM {0.log_table_quoted} order by created_at_utc'
        .format(backend))
